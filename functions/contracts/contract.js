            ...
            if (changeEvent.operationType === "update") {
                ...
                if (keysArray.includes("signatures")) {
                    ...
                    if (signatures && signatures.length > 0) {
                        const endUser = signatures.some(signature => signature.userType === "endUser");
                        if (endUser) {
                            return `${logEntry?.userName || 'Anonymous user'} signed ${fullDocument.title} contract`;
                        }
                    }
                }
            }
            ...
            if (keysArray.includes('signatures')) {
                ...
                if (signatures && signatures.length > 0) {
                    ...
                    if (!hasTenantUser && hasEndUser) {
                        const notificationSummary = createNotificationSummary(changeEvent);
